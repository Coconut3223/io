...
.. image:: ./pics/embedding_4.png
    :scale: 40%
.. image:: ./pics/embedding_5.png
    :scale: 40%
.. image:: ./pics/embedding_6.png
    :scale: 40%
...
- :question: 为什么隐藏层的参数矩阵  :math:`W`  就是 word vector = word embedding？
    从 input 和 target output来看：

    - **input**  :math:`x`  是一个 binary vector，只有 given word(在词袋的索引是 :math:`I`  )是 1， 其余的都是 0.
        | :math:`\implies h=W^Tx` , 只有  :math:`W^T` 的一列 =  :math:`W` 的一行 =  :math:`W_I`  是有用的
        | :math:`\implies h=W_I`  ，传入模型的 就只有第  :math:`I`  那么一个词，也就 参数矩阵  :math:`W`  第  :math:`I`  行  :math:`W_I` 那么一个向量。
        | 所以非常自然地，:math:`\text{word}_I\lrArr W_I\implies \text{words}_{in}\lrArr W_{in}`

    - **target output**  :math:`\hat{y}`  同样是一个 binary vector，只有 target word(在词袋的索引是 :math:`j^*` ) 是 1， 其余都是 0。（和前面类似）
        | :math:`\implies \hat{y}=\text{softmax}(W'^Th)\rightarrow{\text{理想情况}}\begin{cases}\hat{y}_{j^*}=y_{j^*}=1\lrArr W'^T_{j^*}h=1&j= j^* \\\hat{y}_j=y_j=0\lrArr W'^T_jh=0&j\neq j^* \end{cases}` 
        | :math:`\implies`  输出模型的 就只有第  :math:`j^*`  那么一个词，除去代表输入词 的 :math:`h=W_I` , 也就 参数矩阵  :math:`W'` 第  :math:`j^*` 列  :math:`W'^T_{j^*}` 那么一个向量参与到运算。
        | 所以非常自然地， == :math:`\text{word}_{j^*}\lrArr W'^T_{j^*}\implies \text{words}_{out}\lrArr W'^T_{out}` ==
        
        - **输入和输出同是一个词袋**，所以经过这个训练，同时 ==有两种不同== 的嵌入
        - :question: 为什么不看 output？ 、
            因为 output 的值还跟模型能力有关。target output 更能体现理论上的设计。
- :question: 为什么  :math:`W` , :math:`W'`  不一样
    1.  :math:`h=W_I` ，输入完全由  :math:`W_I` 组成；
- :question: 为什么要扔掉  ``contex matrix`` ，只要  ``matirx`` ？
    | 其实选择  :math:`W` 还是 :math:`W'` 作为 word embedding matrix 都是个人选择。
