...
### Origin - Depend on k
 ...
 :math:`` y^*=\max\limits_u\sum\limits_{(x_i,y_i)\in D(\hat{x},\hat{y})}I(\hat{y}=y_i) :math:`` 
...
### Weighted -  Considering the distance
 ...
 :math:`` y^*=\max_u\sum\limits_{(x_i,y_i)\in D(\hat{x},\hat{y})}w_i\cdot I(\hat{y}=y_i) :math:`` 
